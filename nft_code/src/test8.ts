...
class TicketingSystem {
    ...
    async buyTicket(
        { buyer, seller, price,ticketType }: BuyTicketParams
    ): Promise<void> {
        try {
            console.log(`🛒 ${buyer.accountAddress} is buying a ${ticketType} ticket from ${seller.accountAddress} for ${price / 100_000_000} APT`);
            ...
            const tokenDescription = firstMatchingToken.current_token_data?.description;
    ...
    async resellTicket(
        { seller, buyer, resalePrice, organizer, ticketType }: ResellTicketParams
    ): Promise<void> {
        try {
