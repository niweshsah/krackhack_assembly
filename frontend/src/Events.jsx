...
//       const image = event.image ?? 'https://cdn.vectorstock.com/i/1000x1000/82/01/temporary-rubber-stamp-vector-17998201.webp'
//       const title = event.Title ?? "Sample Event"
...
const EventListing = () => {
  ...
  const fallbackEvents = [
    {
      ...
      title: "Summer Music Festival", 
      image: "https://cdn.vectorstock.com/i/1000x1000/82/01/temporary-rubber-stamp-vector-17998201.webp",
      organiser: '0xa53709fb9b81a068f5c4868c62c2513463411995184fb00a7e3c6166b58c9cd1',
      time: "10 PM",
      date: "29th March 2015",
      tickets: [{
        category: "VIP", 
  ...
  const TicketPurchaseModal = ({ event, onClose }) => {
    ...
    // Set the receiver address when modal opens
    useEffect(() => {
      if (event && event.organiser) {
        setReceiverAddress(event.organiser);
      }
    }, [event]);
    ...
    const handlePayment = async () => {
      setLoading(true);
      try {
        const arr = await transfer(receiverAddress, finalTotal);
        setsenderAddr(arr[1]);
        const hash = arr[0];
        setTransactionHash(hash);
        setTransactionSuccess(true);
      ...
      <div className="fixed inset-0 bg-black/80 backdrop-blur-lg flex items-center justify-center z-50 p-4">
        <div className="bg-gray-900 rounded-2xl max-w-md w-full p-6 text-center">
          ...
          <p className="text-gray-300 mb-6">
            ...
            >
              {hash ? `${hash.slice(0, 10)}...` : 'Processing...'}
            </a>
          </p>
          <button
            onClick={onClose}
...
// Helper function for Aptos transfers
const transfer = async (receiverAddress, amount,sender) => {
  if (!receiverAddress) {
  ...
  try {
    ...
    return [response.hash,sender.address];
  } catch (error) {
